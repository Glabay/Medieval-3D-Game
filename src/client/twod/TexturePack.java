...
/**
 * packs several arbitrary sized images into a jPCT texture. the created texture is 2^n x 2^m as required by jPCT. useful for blitting purposes.
 *
 * <p>
 * see <a href='http://www.blackpawn.com/texts/lightmaps/default.html' target='_blank'>this page</a> for an explanation of packing algorithm
 * </p>
 *
...
public class TexturePack {
	 ...
	 *
	 * @param imageType
	 *            must be one of the image types defined in {@link BufferedImage} class
	 * @see BufferedImage
	 */
	...
	/**
	 * packs images into an 2^n x 2^m size d image. after packing clears all references to given images hence this method can only be called once
	 *
	...
	/**
	 * packs image, creates a Texture out of it and adds texture to TextureManager.
	 *
	 * @param useAlpha
	...
	/**
	 * blits one of packed images completely without scaling and with transparency or additional color. see {@link FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color) FrameBuffer.blit(..)} for description of other parameters
	 *
	 * @param buffer
	 *            the FrameBuffer
	 * @param imageId
	 ...
	 *
	 * @return size of blitted image. note this method always returns the same array instance
	 * @see FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color) FrameBuffer.blit(..)
	 */
	public Dimension blit(FrameBuffer buffer, int imageId, int destX, int destY, int transparency, boolean additive, Color color) {
		...
		return blit(buffer, entry, destX, destY, entry.bounds.width, entry.bounds.height, transparency, additive, color);
	}

	/**
	 * blits one of packed images completely with scaling or transparency or additional color see {@link FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color) FrameBuffer.blit(..)} for description of other parameters
	 *
	 * @param buffer
	 *            the FrameBuffer
	 * @param imageId
	 ...
	 * @see FrameBuffer#blit(Texture, int, int, int, int, int, int, int, int, int, boolean, Color) FrameBuffer.blit(..)
	 */
	public Dimension blit(FrameBuffer buffer, int imageId, int destX, int destY, int destWidth, int destHeight, int transparency, boolean additive, Color color) {
	 ...
	 *            id of packed image as returned by addImage(..)
	 * @return size of blitted image. note this method always returns the same array instance
	 * @see FrameBuffer#blit(Texture, int, int, int, int, int, int, boolean) FrameBuffer.blit(..)
	 */
	public Dimension blit(FrameBuffer buffer, int imageId, int destX, int destY, boolean transparent) {
		...
		Entry entry = entries.get(imageId);
		buffer.blit(texture, entry.bounds.x, entry.bounds.y, destX, destY, entry.bounds.width, entry.bounds.height, transparent);

		blittedSize.setSize(entry.bounds.width, entry.bounds.height);
		return blittedSize;
	}

	private Dimension blit(FrameBuffer buffer, Entry entry, int destX, int destY, int destWidth, int destHeight, int transparency, boolean additive, Color color) {

		buffer.blit(texture, entry.bounds.x, entry.bounds.y, destX, destY, entry.bounds.width, entry.bounds.height, destWidth, destHeight, transparency, additive, color);
	...
	/**
	 * returns the closest power of two that is equal or greater than given number
	 */
